...
def test_should_return_first_five_observation_initials(test_app: TestClient):
    response = test_app.get("/api/climsoft/v1/observation-initials", params={"limit": 5})
...
def test_should_return_single_observation_initial(test_app: TestClient, get_observation_initial: climsoft_models.Observationinitial):
    response = test_app.get(f"/api/climsoft/v1/observation-initials/{get_observation_initial.recordedFrom}/{get_observation_initial.describedBy}/{get_observation_initial.obsDatetime}/{get_observation_initial.qcStatus}/{get_observation_initial.acquisitionType}")
    assert response.status_code == 200
    response_data = response.json()
    print(response_data)
...
def test_should_create_a_observation_initial(test_app: TestClient, get_station: climsoft_models.Station, get_obselement: climsoft_models.Obselement):
    ...
    response = test_app.post("/api/climsoft/v1/observation-initials", data=json.dumps(observation_initial_data, default=lambda x: x.strftime("%Y-%m-%d %H:%M:%S")))
    assert response.status_code == 200
    response_data = response.json()
    print(response_data)
    assert len(response_data["result"]) == 1
...
def test_should_raise_validation_error(test_app: TestClient, get_station: climsoft_models.Station, get_obselement: climsoft_models.Obselement):
    ...
    response = test_app.post("/api/climsoft/v1/observation-initials", data=json.dumps(observation_initial_data, default=str))
...
def test_should_update_observation_initial(test_app: TestClient, get_observation_initial: climsoft_models.Observationinitial):
    ...
    response = test_app.put(f"/api/climsoft/v1/observation-initials/{get_observation_initial.recordedFrom}/{get_observation_initial.describedBy}/{get_observation_initial.obsDatetime}/{get_observation_initial.qcStatus}/{get_observation_initial.acquisitionType}", data=json.dumps(updates, default=str))
    response_data = response.json()
...
def test_should_delete_observation_initial(test_app: TestClient, get_observation_initial: climsoft_models.Observationinitial):
    ...
    response = test_app.delete(f"/api/climsoft/v1/observation-initials/{get_observation_initial.recordedFrom}/{get_observation_initial.describedBy}/{get_observation_initial.obsDatetime}/{get_observation_initial.qcStatus}/{get_observation_initial.acquisitionType}")
    assert response.status_code == 200
